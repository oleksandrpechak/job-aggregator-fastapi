...
SOURCE_URL = "https://jobs.dou.ua/vacancies/?category=Python&exp=0-1"
SOURCE_NAME = "dou"

@celery_app.task(name="app.tasks.ingest_dou_jobs")
def ingest_dou_jobs():
    response = requests.get(SOURCE_URL, headers={"User-Agent": "job-aggregator-bot/0.1"})
    ...
    for item in listings:
        ...
        dedup_key = build_dedup_key(SOURCE_NAME, link)
        job_doc = {
            "title": title,
            "company": company,
            "level": "junior",
